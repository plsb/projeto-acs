...
public class ImportarXML extends Activity {
	...
	public void ImportaUsuarios(){
		...
		try {			
			if (!(xml.carregar("scs.xml","usuario") == null)){		
				//I M P O R T A Ç Ã O   D E   U S U � R I O S
	...
	public void ImportaResidencias(){
		...
		try {
			...
			System.out.println("Erro Importando Residências: "+e.getMessage());
		} catch (IOException e) {
			System.out.println("Erro Importando Residências: "+e.getMessage());
		} catch (JDOMException e) {
			System.out.println("Erro Importando Residências: "+e.getMessage());
	...
	public void ImportaHa(){
		//I M P O R T A Ç Ã O   D O S  A C O M P A N H A M E N T O S  H I P E R T E N S � O		
		try {
